...
import { UserDetailsModal } from "../../customers/components/UserDetailsModal";
      ...
      <Section spacing="md" className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 animate-fade-in opacity-0" style={{ animationDelay: '50ms' }}>
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 shrink-0 overflow-hidden rounded-full border border-border-strong shadow-sm bg-surface">
              <AdminImage
                src={user?.avatarUrl}
                alt={displayName}
            ...
            <div>
              <h2 className="text-2xl font-bold text-text">Chào mừng trở lại, {displayName}! 👋</h2>
                    ...
                    dashboard.recentOrders.map((order, index) => (
                      <tr key={order.id} className="animate-fade-in-up opacity-0 transition-colors hover:bg-surface-alt" style={{ animationDelay: `${350 + index * 40}ms` }}>
                        ...
                        <td className="px-5 py-4 font-medium text-text">
                          ...
                          >
                            <div className="h-8 w-8 shrink-0 overflow-hidden rounded-full border border-border bg-surface-alt transition-transform duration-200 group-hover:scale-110 group-hover:border-primary/50 group-hover:shadow-sm">
                              <AdminImage
                                src={order.userAvatarUrl}
                                alt={order.recipientName}
                                fallbackLabel={order.recipientName}
                                className="h-full w-full object-cover"
                                fallbackClassName="flex h-full w-full items-center justify-center bg-primary-soft text-xs font-bold text-primary"
                              />
                            </div>
                            <span className="transition-colors duration-200 group-hover:text-primary">{order.recipientName ?? "-"}</span>
                          </div>
                        </td>
              ...
              <div className="mt-4 space-y-3">
                {loading ? (
                  ...
                  dashboard.topProducts.slice(0, 5).map((product, index) => (
                    <Link key={product.productId} to={`/admin/products/${product.productId}/edit`} className="animate-fade-in-up flex items-center gap-3 rounded-xl border border-border p-3 opacity-0 transition-all hover:shadow-md hover:border-primary/30 group" style={{ animationDelay: `${450 + index * 50}ms` }}>
                      <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-surface-alt border border-border/50 group-hover:border-primary/20 transition-colors">
                        <AdminImage src={product.thumbnailUrl ?? ""} alt={product.productName} className="h-full w-full object-cover" fallbackClassName="flex h-full w-full items-center justify-center text-xs font-bold bg-success-soft text-success" fallbackLabel={product.productName} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-bold text-text group-hover:text-primary transition-colors">{product.productName}</p>
