        ...
        <div className="text-center shrink-0 w-32">
          ...
          <div className="flex justify-center text-amber-400 mb-1">
            {[1, 2, 3, 4, 5].map((item) => (
              <Star 
                key={item} 
                className={`w-4 h-4 ${item <= Math.round(reviewSummary?.averageRating || 0) ? 'fill-current' : 'text-gray-200 fill-gray-200'}`} 
              />
            ))}
          </div>
            ...
            <div key={review.id} className="pb-6 border-b border-gray-50 last:border-0 last:pb-0">
              ...
              <div className="flex text-amber-400 mb-3">
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star 
                    key={index} 
                    className={`w-3 h-3 ${index < review.rating ? 'fill-current' : 'text-gray-200 fill-gray-200'}`} 
                  />
                ))}
              </div>
      ...
      {reviewsPage && !reviewsPage.last && (
        <div className="mt-6 text-center">
          <button
            ...
            disabled={loading}
            className="px-6 py-2 border border-blue-600 text-blue-600 rounded-lg font-medium hover:bg-blue-50 transition-colors disabled:opacity-50"
          >
