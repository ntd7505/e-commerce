...
export default function Authority() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [userData, roleData] = await Promise.all([getAdminUsers(), getRoles()]);
      setUsers(userData);
      setRoles(roleData);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-6 pb-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-[22px] font-bold text-gray-900">Control Authority</h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {loading ? (
          <div className="col-span-4 rounded-xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-400">
            Loading roles...
          </div>
        ) : (
          roles.map((role) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-[12px] font-bold uppercase text-gray-500">{role.name}</p>
                </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 p-5">
          ...
          <div className="relative w-72">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-[13px] outline-none transition focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[13px]">
            <thead className="sticky top-0 bg-gray-50 text-[11px] uppercase tracking-wider text-gray-500">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-5 py-10 text-center text-gray-400">Loading users...</td>
                </tr>
              ) : filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-5 py-10 text-center text-gray-400">No users found.</td>
                </tr>
              ) : (
