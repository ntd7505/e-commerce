        ...
        <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <div className="mb-6 flex items-center justify-between">
                ...
                <button
                    type="button"
                    onClick={onAdd}
                    className="flex items-center gap-2 rounded-2xl border border-slate-100 bg-white px-3 py-2 text-[12px] font-bold text-slate-600 shadow-sm hover:bg-slate-50"
                >
            ...
            <div className="space-y-4">
                {variants.map((variant, index) => (
                    <div key={variant.id ?? `new-${index}`} className="rounded-lg border border-slate-200 p-4">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
                            <input
                                value={variant.variantName}
                                onChange={(event) => onChange(index, { variantName: event.target.value })}
                                placeholder="Variant name"
                                className="rounded-lg border border-slate-200 bg-[#f8f9fa] px-3 py-2.5 text-[13px] font-semibold focus:border-emerald-500 focus:outline-none md:col-span-2"
                            />
                            <input
                                type="number"
                                ...
                                placeholder="Stock"
                                className="rounded-lg border border-slate-200 bg-[#f8f9fa] px-3 py-2.5 text-[13px] font-semibold focus:border-emerald-500 focus:outline-none"
                            />
                            <input
                                type="number"
                                value={variant.price}
