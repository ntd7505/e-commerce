...
public interface HomeBannerRepository extends JpaRepository<HomeBanner, Long> {
    ...
    @Query("SELECT b FROM HomeBanner b WHERE b.deleted = false " + "AND (:position IS NULL OR b.position = :position) "
            + "AND (:active IS NULL OR b.isActive = :active)")
    Page<HomeBanner> findAdminBanners(
            @Param("position") BannerPosition position, @Param("active") Boolean active, Pageable pageable);

    @Query("SELECT DISTINCT b FROM HomeBanner b " + "JOIN FETCH b.product p "
