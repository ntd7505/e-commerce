...
export default function AdminProfileOverview() {
  ...
  if (loading) {
    return <div className="mx-auto max-w-[900px] rounded-xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-500">Loading profile...</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-[900px] space-y-6 pb-10">
      <div>
        <h2 className="text-[22px] font-bold text-gray-900">Admin Profile</h2>
        <p className="mt-1 text-[13px] text-gray-500">Thông tin tài khoản đang đăng nhập.</p>
      </div>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-gray-100 bg-gray-50 p-4">
            <div className="flex items-center gap-2 text-[12px] font-bold uppercase text-gray-400">
              <Mail className="h-4 w-4" />
              Email
            </div>
            <p className="mt-2 text-[14px] font-bold text-gray-900">{user.email}</p>
          </div>
          <div className="rounded-lg border border-gray-100 bg-gray-50 p-4">
            <div className="flex items-center gap-2 text-[12px] font-bold uppercase text-gray-400">
            ...
            <p className="mt-2 text-[14px] font-bold text-gray-900">{user.phoneNumber}</p>
          </div>
        </div>

        <div className="mt-6">
          <div className="mb-3 flex items-center gap-2 text-[12px] font-bold uppercase text-gray-400">
            <ShieldCheck className="h-4 w-4" />
            Roles
          </div>
          <div className="flex flex-wrap gap-2">
            {user.roles?.map((role) => (
              <span key={role.name} className="rounded-md bg-gray-100 px-2.5 py-1 text-[12px] font-bold text-gray-700">
                {role.name}
              </span>
            ))}
          </div>
