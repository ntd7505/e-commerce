...
export default function RoleManagementPageContent() {
  ...
  const loadData = async () => {
    try {
      ...
      setError("Không th? t?i d? li?u phân quy?n");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRole = async (e: React.FormEvent) => {
    ...
    if (!formValues.name.trim() || !formValues.description.trim()) {
      setError("Vui lòng nh?p tên và mô t? role"); return;
    }
    if (formValues.permissions.length === 0) {
      setError("Vui lòng ch?n ít nh?t m?t permission"); return;
    }
    try {
      setSaving(true);
      ...
      setError("");
      setSuccessMsg(`Role "${created.name}" dã du?c t?o thành công!`);
      setTimeout(() => setSuccessMsg(""), 3000);
    } catch {
      setError("Không th? t?o role. Ki?m tra tên ho?c permissions.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="mx-auto max-w-[1400px] space-y-6 pb-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Qu?n lý vai trò và quy?n h?n trong h? th?ng.</p>
        </div>
        <button
          type="button"
          onClick={loadData}
