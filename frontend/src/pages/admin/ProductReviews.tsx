    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Read customer reviews across products. Moderation API is not available yet.</p>
        </div>
        <button
          type="button"
          onClick={loadReviews}
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 p-5">
          <div>
            <h3 className="font-bold text-slate-900">Review List</h3>
            <p className="mt-1 text-xs font-medium text-slate-500">Showing {filteredReviews.length} of {reviews.length} reviews</p>
          </div>
              ...
              {ratingOptions.map((option) => (
                <button
                  ...
                  className={`rounded-md px-3 py-1.5 text-xs font-bold transition-colors ${
                    ratingFilter === option.value ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500 hover:text-slate-800"
                  }`}
                >
                  {option.label}
                </button>
        ...
        {loading && <div className="p-6 text-sm text-slate-500">Loading reviews...</div>}
        {!loading && error && <div className="p-6 text-sm font-semibold text-red-600">{error}</div>}
        {!loading && !error && filteredReviews.length === 0 && <div className="p-6 text-sm text-slate-500">No reviews found.</div>}
            ...
            {filteredReviews.map((review) => (
              <div key={review.id} className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="font-bold text-slate-900">{review.productName}</p>
                  ...
                  <div className="flex items-center gap-1 text-amber-500">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <Star key={index} className={`h-4 w-4 ${index < (Number(review.rating) || 0) ? "fill-current" : "text-slate-300"}`} />
                    ))}
                  </div>
                </div>

                <div className="mt-4">
                  <p className="font-semibold text-slate-900">{review.title || "Untitled review"}</p>
                  <p className="mt-2 text-sm text-slate-600">{review.content || "No content"}</p>
                </div>

                {(review.media ?? []).length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-3">
                    {(review.media ?? []).map((media) => (
                      <a key={media.id} href={media.url} target="_blank" rel="noreferrer" className="block h-20 w-20 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                        {media.mediaType === "IMAGE" ? (
                          <AdminImage src={media.url} alt="Review media" />
                        ) : (
                          <div className="flex h-full w-full items-center justify-center text-xs font-bold text-slate-500">VIDEO</div>
                        )}
                      </a>
                    ))}
                  </div>
